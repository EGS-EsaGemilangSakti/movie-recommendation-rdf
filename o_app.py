...
if option == "Search for a Movie":
    ...
    if st.button("Show Results"):
        ...
        SELECT ?name ?overview ?release_date ?runtime ?budget ?revenue ?director ?movie_id ?actor_name ?country ?language ?genre
        WHERE {{
            ?movie schema:name "{movie_title}"^^xsd:string ;
        ...
        if movie_data:
            ...
            SELECT ?name ?overview ?movie_id
            WHERE {{
                ?movie schema:name ?name ;
                       schema:description ?overview ;
                       schema:director <{director_uri}> ;
                       schema:id ?movie_id .
                FILTER(?name != "{movie_title}"^^xsd:string)
            }}
            LIMIT 5
            """
            ...
            if director_recommendations:
                ...
                for i, recommendation in enumerate(director_recommendations):
                    movie_name = recommendation.get("name", {}).get("value", "N/A")
            ...
            actor_query = f"""
                            ...
                            SELECT DISTINCT ?name ?overview ?movie_id
                            WHERE {{
                                ?movie schema:name "{movie_title}"^^xsd:string ;
                                    schema:actor ?actor .
                                ?other_movie schema:actor ?actor ;
                                            schema:name ?name ;
                                            schema:description ?overview ;
                                            schema:id ?movie_id .
                                FILTER(?name != "{movie_title}"^^xsd:string)
    ...
    if st.button("Show Results"):
        if selected_question == "Top 10 Movies by Budget":
            ...
            WHERE {
                ...
                FILTER(LANG(?genre) = "en" && ?genre != "" && ?genre != "N/A")  # Filter out empty or "N/A" genres
            }
            GROUP BY ?genre
            ORDER BY DESC(?count)
            LIMIT 10
            ...
            WHERE {
                ...
                FILTER(LANG(?country) = "en" && ?country != "" && ?country != "N/A")  # Filter out empty or "N/A" countries
            }
            GROUP BY ?country
            ORDER BY DESC(?count)
            LIMIT 10
            ...
            SELECT ?name (COUNT(?country) AS ?country_count) ?movie_id
            WHERE {
                ?movie schema:name ?name ;
                    schema:countryOfOrigin ?country ;
                    schema:id ?movie_id .
            ...
            for row_idx in range(rows):
                for col_idx in range(5):
                    ...
                    if index < len(results):
                        ...
                        movie_name = result.get("name", {}).get("value", "N/A")
